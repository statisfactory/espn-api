from .constant import POSITION_MAP, PRO_TEAM_MAP, PLAYER_STATS_MAP, ESPN_POSITION_MAP
...
class BoxPlayer(Player):
    ...
    def __init__(self, data, pro_schedule, positional_rankings, week, year):
        ...
        self.posId = None  # Initialize posId attribute
        self.mapped_position = None  # Initialize mapped position attribute

        if 'lineupSlotId' in data:
            self.slot_position = POSITION_MAP[data['lineupSlotId']]

        player = data['playerPoolEntry']['player'] if 'playerPoolEntry' in data else data['player']
        self.posId = player['defaultPositionId']  # Set posId attribute
        self.mapped_position = ESPN_POSITION_MAP.get(self.posId, 'Unknown')  # Map posId to position name
        ...
        if player['proTeamId'] in pro_schedule:
            ...
            posId_str = str(self.posId)
            if posId_str in positional_rankings:
                self.pro_opponent = PRO_TEAM_MAP[opp_id]
                self.pro_pos_rank = positional_rankings[posId_str][str(opp_id)] if str(opp_id) in positional_rankings[posId_str] else 0
        else: # bye week
